...
def constructor (data, nrows):
    try:
        ...
        # Algortmo OPTICS
        TRACLUS_map_OPTICS, TRACLUS_map_df_OPTICS, tabla_OPTICS, n_clusters = get_cluster_trajectories(df=df, trajectories=tray, clustering_algorithm=OPTICS)
        # print(n_clusters)

        # Algortmo HDBSCAN
        TRACLUS_map_HDBSCAN, TRACLUS_map_df_HDBSCAN, tabla_HDBSCAN, _ = get_cluster_trajectories(df=df, trajectories=tray, clustering_algorithm=HDBSCAN)
        
        # Algortmo DBSCAN
        TRACLUS_map_DBSCAN, TRACLUS_map_df_DBSCAN, tabla_DBSCAN, _ = get_cluster_trajectories(df=df, trajectories=tray, clustering_algorithm=DBSCAN, max_eps=0.1)

        # Algortmo SpectralClustering
        TRACLUS_map_SpectralClustering, TRACLUS_map_df_SpectralClustering, tabla_SpectralClustering,  _ = get_cluster_trajectories(df=df, trajectories=tray, clustering_algorithm=SpectralClustering, n_clusters = n_clusters , affinity = 'nearest_neighbors')

        # Algortmo AgglomerativeClustering
        TRACLUS_map_AgglomerativeClustering, TRACLUS_map_df_AgglomerativeClustering, tabla_AgglomerativeClustering, _ = get_cluster_trajectories(df=df, trajectories=tray,clustering_algorithm=AgglomerativeClustering, n_clusters = n_clusters, linkage = 'ward')
